...
\end{verbatim}
              ...
              By default, \Yaws\ will perform a 302 redirect. The HTTP status code
              can be changed using the \verb+code+ parameter. Note that the
              status code must be known by \Yaws\.\\
              For 3xx status codes, the \verb+URL+ parameter must be present and
              will be used to build the new location.\\
              For other status codes (1xx, 2xx, 4xx and 5xx), it can be
              omitted. In the absence of \verb+URL+, \Yaws\ will return a
              generic response with the specified status code. Else, the
              \verb+URL+ parameter must be a relative url and will be used to
...
\end{verbatim}
              ...
              When we specify a relative url as target for the redirect, the
              redir will be to the current http(s) server.
